...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final int baseBottomHeight = getResources().getDimensionPixelSize(R.dimen.bottom_bar_height);
        final View bottomRow = findViewById(R.id.bottom_bar_row);
        ViewCompat.setOnApplyWindowInsetsListener(root, (v, insets) -> {
            ...
            if (bottom != null) {
                ...
                if (lp != null) {
                    int newHeight = baseBottomHeight + bars.bottom;
            ...
            if (bottomRow != null) {
                bottomRow.setPadding(bottomRow.getPaddingLeft(), bottomRow.getPaddingTop(), bottomRow.getPaddingRight(), bars.bottom);
            }
        ...
        // Open detailed expense report when tapping "View entries"
        View btnViewReports = findViewById(R.id.btn_view_reports);
        if (btnViewReports != null) {
            long finalUserId1 = userId;
            btnViewReports.setOnClickListener(v ->
                    showFragment(ExpenseReportFragment.newInstance(finalUserId1)));
        }
        ...
        View btnManageBudget = findViewById(R.id.btn_manage_budget);
        if (btnManageBudget != null) {
            long finalUserIdManage = userId;
            btnManageBudget.setOnClickListener(v -> {
                Intent intent = new Intent(this, ManageBudgetsActivity.class);
                intent.putExtra(ManageBudgetsActivity.EXTRA_USER_ID, finalUserIdManage);
                startActivity(intent);
            });
        }

        scheduleBackgroundWork(userId);
        scheduleDailyNotifications(userId);
    }

    private void scheduleBackgroundWork(long userId) {
    ...
    public void showFragment(Fragment f) {
        ...
        else if (f instanceof com.example.personalfinancialmanagement.SettingsFragment) setNavSelected("settings");
            ...
            runOnUiThread(() -> {
                ...
                if (tvBalanceHint != null) {
                    tvBalanceHint.setText(net >= 0
                            ? getString(R.string.overview_balance_hint_positive)
                            : getString(R.string.overview_balance_hint_negative));
                }
                ...
                if (tvLastRefresh != null) {
                    tvLastRefresh.setText(getString(R.string.overview_last_updated,
                            timeFmt.format(new java.util.Date(now))));
                }
                ...
                if (tvBudgetSummary != null) {
                    if (fBudgetTotal > 0) {
                        String used = String.format(locale, "$%,.2f", fTotalExpense);
                        String total = String.format(locale, "$%,.2f", fBudgetTotal);
                        tvBudgetSummary.setText(getString(R.string.overview_budget_summary, used, total));
                    } else {
                        tvBudgetSummary.setText(R.string.overview_budget_none);
                    }
                }
                if (tvBudgetRemaining != null) {
                    if (fBudgetTotal > 0) {
                        double remaining = fBudgetTotal - fTotalExpense;
                        String amount = String.format(locale, "$%,.2f", Math.abs(remaining));
                        int resId = remaining >= 0 ? R.string.overview_budget_remaining_positive : R.string.overview_budget_remaining_over;
                        tvBudgetRemaining.setText(getString(resId, amount));
                    } else {
                        tvBudgetRemaining.setText(R.string.overview_budget_create_prompt);
                    }
                }
                ...
                if (rv2 != null && rv2.getAdapter() instanceof CombinedEntryAdapter) {
                    ...
                    if (tvActivitySummary != null && tvActivityAmount != null && tvActivityHint != null) {
                        if (!items.isEmpty()) {
                            ...
                            tvActivitySummary.setText(top.income ? R.string.overview_activity_latest_income : R.string.overview_activity_latest_expense);
                            tvActivityAmount.setText(String.format(locale, "%s $%,.2f", top.income ? "+" : "-", top.amount));
                            String description = (top.title == null || top.title.trim().isEmpty())
                                    ? getString(R.string.overview_entry_unnamed)
                                    : top.title;
                            String timeLabel = timeFmt.format(new java.util.Date(top.time));
                            tvActivityHint.setText(getString(R.string.overview_activity_hint_pattern, description, timeLabel));
                        } else {
                            tvActivitySummary.setText(R.string.overview_activity_summary_placeholder);
                            tvActivityAmount.setText(R.string.overview_activity_amount_placeholder);
                            tvActivityHint.setText(R.string.overview_activity_empty_hint);
                        }
                    }
                }
            });
